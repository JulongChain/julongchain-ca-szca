								...
								<c:choose>
									...
									<c:otherwise>
										...
										<div class="control-group">
											...
											<div class="controls">
												<input type="hidden" name="certindex" id="certindex" value="${certindex }"/>
												<input type="hidden" name="certsize" id="certsize" value="${certsize }"/>
												...
												<c:if test="${certindex gt 0 }">
													<button type="button" class="btn blue" onclick="previousCert('${viewcertificateVo.usernameVal}')">上一个</button>
												</c:if>
												<c:if test="${certindex lt (certsize - 1) }">
													<button type="button" class="btn blue" onclick="nextCert('${viewcertificateVo.usernameVal}')">下一个</button>
												</c:if>
											</div>
										</div>
										...
										<div class="control-group">
											...
											<div class="controls">
												<input type="text" name="sha1Finger" id="sha1Finger" value="${viewcertificateVo.fingerprint_sha1Val}" readonly="readonly" class="span6 m-wrap popovers" data-trigger="hover"   />
											</div>
										</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													<c:if test="${!isRevoked}">
														...
														<button type="button" class="btn blue" onclick="revoke()">${viewcertificateVo.revoke}证书</button>
														<c:if test="${certsize gt 1 }">
															<button type="button" class="btn blue" onclick="revokeUser()">${viewcertificateVo.revoke}用户</button>
														</c:if>
...
<script type="text/javascript">
    function revoke(){
        ...
        $.ajax({
            ...
            async:true,
            dataType:'json',
            data:{username:$("#username").val(),reason:$("#selectrevocationreason").val(),sh1Finger:$("#sha1Finger").val()},
    ...
    function revokeUser(){
        ...
        $.ajax({
            type : "POST",
            url : "${pageContext.request.contextPath}/ra/revokeUser.html",
            async:true,
            dataType:'json',
    ...
    function nextCert(username){
        var curIndex = parseInt($('#certindex').val());
        var certsize = parseInt($('#certsize').val());
        if(curIndex < (certsize - 1)){
            window.location.href="${pageContext.request.contextPath}/ra/viewcertificate.html?index=" + (curIndex + 1) + "&username="+encodeURI(username);
        }
    }

    function previousCert(username){
        var curIndex = parseInt($('#certindex').val());
        if(curIndex > 0){
            window.location.href="${pageContext.request.contextPath}/ra/viewcertificate.html?index=" + (curIndex - 1) + "&username="+encodeURI(username);
        }
