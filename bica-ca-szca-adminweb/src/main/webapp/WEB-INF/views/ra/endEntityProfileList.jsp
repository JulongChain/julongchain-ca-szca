			...
			<div class="container-fluid">
				<div class="row-fluid">
					<div class="span12">
						...
						<h3 class="page-title">
							终端实体模板列表<small></small>
						</h3>
						<ul class="breadcrumb">
							<li><i class="icon-home"></i> <a href="#">终端实体模板</a> <i class="icon-angle-right"></i></li>
							<li><a href="#">终端实体模板列表</a></li>
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption">
									<i class="icon-reorder"></i> <span class="hidden-480">当前终端实体模板列表</span>
								</div>
								<div></div>
							</div>
							<div class="portlet-body">
								...
								<table class="table table-striped table-bordered table-hover table-full-width ">
									...
									<!-- 
									<c:forEach items="${endEntityProfiles }" var="defaultProfile">
										<tr>
									...
									<tr>
										...
										<td>
											EMPTY
											<input type="hidden" name="profileNames" value="EMPTY"/>
										</td>
										<td colspan="5">
											<input type="button" name="updateBtn2" disabled="disabled" class="btn white" value="编辑" onclick="">
											<input type="button" name="updateBtn3" disabled="disabled" class="btn white" value="删除" onclick="">
											<input type="button" name="updateBtn4" disabled="disabled" class="btn white" value="重命名" onclick="">
											<input type="button" name="updateBtn5" class="btn blue" value="复制" onclick="addEEProfile()">
										</td> 
									</tr>
									<c:forEach items="${endEntityProfiles }" var="endEntityProfile">
										<tr>
											<td>${endEntityProfile.key}</td>
											<td>
												${endEntityProfile.value} 
												<input type="hidden" name="profileNames" value="${endEntityProfile.value}"/>
											</td>
											<td colspan="5">
												<input type="button" name="updateBtn2" class="btn blue" value="编辑" onclick="editEEProfile('${endEntityProfile.value }')">
												<input type="button" name="updateBtn3" class="btn blue" value="删除" onclick="removeEEProfile('${endEntityProfile.value }')">
												<input type="button" name="updateBtn4" class="btn blue" value="重命名" onclick="renameEEProfile('${endEntityProfile.value }')">
												<input type="button" name="updateBtn5" class="btn blue" value="复制" onclick="cloneEEProfile('${endEntityProfile.value }')">
											</td>
										</tr>
									</c:forEach>
									<tr>
											...
											<td colspan="5">
												<input type="button" class="btn blue" value="新建模板" onclick="addEEProfile()"/>
											</td>
										</tr>
								</table> 
							</div> 
...
function addEEProfile(){
	...
	window.location.href ="${pageContext.request.contextPath}/ra/addEndEntityProfile.html?profileName=" + profileName;
} 
...
function cloneEEProfile(orgname){
	var profileName = document.getElementById("profileName").value;
	if(profileName == ''){
		alert('请指定模板名称！');
		return false;
	...
	window.location.href ="${pageContext.request.contextPath}/ra/cloneEndEntityProfile.html?orgName=" + orgname + "&profileName=" + profileName;
} 
function removeEEProfile(name){
	if(window.confirm('确定删除证书模板？')){
		window.location.href ="${pageContext.request.contextPath}/ra/removeEndEntityProfile.html?profileName=" + name;
    }
	
} 

function editEEProfile(n){
	window.location.href ="${pageContext.request.contextPath}/ra/editEndEntityProfile.html?profileName=" + n;
} 


function renameEEProfile(oldName){
	...
	if(window.confirm('确定重命名证书模板？')){
		window.location.href ="${pageContext.request.contextPath}/ra/renameEndEntityProfile.html?orgName=" + oldName + "&profileName=" + newName;
	}
}  
</script>
<!-- END BODY -->
