...
<body class="page-header-fixed">
	...
	<div class="page-container">
		...
		<jsp:param name="menuLevel2" value="editendentity" />
		...
		<div class="page-content">
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							...
							<div class="portlet-title">
								...
								<div class="caption"><i class="icon-reorder"></i>编辑终端实体</div>
							...
							<div class="portlet-body form">
								...
								<form action="${pageContext.request.contextPath}/ra/changeUserData.html"  method="post" id="formId"  class="form-horizontal">
								<c:choose>
								 ...
								 <input type="hidden" name="username" id="username" value="" />
  								  <input type="hidden" name="primevalUsername" id="primevalUsername" value="${USERNAME}" />
 								  <input type="hidden" name="subjectDN"  id="subjectDN" value="" />
 									...
 									<input type="hidden" name="endentityprofileid" id="endentityprofileid" value="${profileid}" />
									...
									<hr />
                                     <c:if test="${useNameFields}"> 
 										...
 										<c:if test="${isModifyableUsername}"> 
  											<input type="text" name="textfieldusername" id="textfieldusername" value="${USERNAME}" class="span6 m-wrap popovers" data-trigger="hover" data-content="" data-original-title="" />
  										</c:if>
  										${isRequiredUsername?"<b class='bclass'> *</b>":""}
   										</div>
  									</div>
                                     </c:if>    
                                    ...
                                    <c:if test="${useCleartextPasswordFields}"> 
 										...
 										<div class="controls">
 									   <input type="checkbox" name="checkboxcleartextpassword" id="checkboxcleartextpassword" value="true" tabindex="" ${CLEARTEXTPASSWORD eq "true"?"checked='checked'":""}    ${isRequiredCleartextPassword?"disabled='disabled'":""}  id="checkboxcleartextpassword" />
 										使用
      								    </div>
  									    </div>
                                        </c:if>   
                                ...
                                <c:if test="${useEmailFields}"> 
 										...
 										<div class="controls">
 									   <input type="text" name="textfieldemail" id="textfieldemail"  class="span3 m-wrap popovers" data-trigger="hover"  maxlength="255" tabindex="" value="${emailname}"> @
                           	        ...
                           	        <c:otherwise> 
                            	        	<select class="span6 m-wrap" name="selectemaildomain" id="selectemaildomain" data-placeholder="Choose a Category" tabindex="1">
											    <c:forEach var="emailoption" items="${emailoptions}" varStatus="status">
												  <option value="${nameoption}"  ${emaildomain eq nameoption?"selected='selected'":""}  >${nameoption}</option>
												  </c:forEach>
											</select>
                             	   </c:otherwise>
                                       </c:choose>
 										...
 										<c:if test="${isModifyableEmail}"> 
 										   <input type="text" name="textfieldemaildomain"   maxlength="255" tabindex=""  value="${emaildomain}" class="span3 m-wrap popovers" data-trigger="hover" data-content="" data-original-title="" title=""/>
   										</c:if>
           ...
           <hr />
                      <!-- ---------- Subject DN  BEGIN------------------- -->
                                     ...
                                     <strong>${subjectDn.options[0]}</strong>
                           	        ...
                           	        <c:otherwise> 
                            	        	<select class="span6 m-wrap" name="selectsubjectdn${status.index}" data-placeholder="Choose a Category" tabindex="1">
												...
												<option value="${sunbjectDnOption}"   ${subjectDn.subjectDNField eq sunbjectDnOption?"selected='selected'":""}   >${sunbjectDnOption}</option>
												  </c:forEach>
											</select>
                             	   </c:otherwise>
                                       </c:choose>
  								       ...
  								       <c:if test="${subjectDn.modifyable}">
  							       <input type="text"  name="selectsubjectdn${status.index}" size="40" maxlength="255" tabindex=""  class="span6 m-wrap popovers" data-trigger="hover"  value="${subjectDn.subjectDNField}"
  							       ...
  							       title="Must match format specified in profile. / Technical detail - the regex is ${subjectDn.regex}"/>
  									 ...
  									 <c:if test="${subjectDn.fieldOfType}">
  									   ...
  									   <input type="checkbox" name="textfieldsubjectdn${status.index}" value="true"  class="span6 m-wrap popovers" data-trigger="hover" tabindex="" 
  									    <c:if test="${ not empty subjectDn.subjectDNField}"> 	
  									    checked="checked" 
   									    </c:if> 
  									    <c:if test="${subjectDn.required}"> 	
    									    disabled="disabled" 
  									    </c:if>
    									    id="checkboxsubjectdn${status.index}" />
    									   </c:if> 
    						   ...
    						   <c:if test="${otherSubjectVo.use}">
    							<input type="checkbox" name="checkboxsubjectaltname" value="true"  class="span6 m-wrap popovers" data-trigger="hover" tabindex=""
     							 <c:if test="${otherSubjectVo.required}"> 	
    							      ...
    							      <c:if test="${fn:contains(otherSubjectVo.subjectDNField, '@')}">
					            	  	 <c:set var="length" value="${fn:length(otherSubjectVo.subjectDNField)}" />
					            	   <c:set var="index" value="${fn:indexOf(otherSubjectVo.subjectDNField,'@')}" />
 					            	  <c:set var="name" value=" ${fn:substring(otherSubjectVo.subjectDNField, 0,index)}" />
					            	  <c:set var="domain" value=" ${fn:substring(otherSubjectVo.subjectDNField, index+1,length)}" />
 					            	  </c:if> 
  					            	 <c:if test="${!fn:contains(otherSubjectVo.subjectDNField, '@')}">
 					            	   	  <c:set var="domain" value="${otherSubjectVo.subjectDNField}" />
  					               </c:if> 
   							     <c:if test="${!otherSubjectVo.use}">
   							       ...
   							       <c:if test="${otherSubjectVo.modifyable}">
 					            		<input type="text" name="textfieldsubjectaltname${status.index}" size="15" maxlength="255" tabindex=""  class="span6 m-wrap popovers" data-trigger="hover" title="" value='${domain}'  />
   							         </c:if>
   							     <c:if test="${!otherSubjectVo.modifyable}">  
   							     	  <c:choose>
                                     ...
                                     <strong>${otherSubjectVo.rfc822NameOptions[0]}</strong>
                           	        ...
                           	        <c:otherwise> 
                            	        	<select class="span6 m-wrap" name="selectsubjectaltname${status.index}" data-placeholder="Choose a Category" tabindex="1">
												...
												<option value="${rfc822NameOption}"  ${otherSubjectVo.subjectDNField eq rfc822NameOption?"selected='selected'":""}>${rfc822NameOption}</option>
												  </c:forEach>
											</select>
                             	   </c:otherwise>
                                       </c:choose>
 					            	 ...
 					            	 <c:if test="${!fn:contains(otherSubjectVo.subjectDNField, '@')}">
  									    ...
  									    <input type="text" name="textfieldupnname" size="20" maxlength="255"  class="span6 m-wrap popovers" value="${name}" data-trigger="hover" tabindex="" /> @
  									     </c:if>
  									   <c:choose>
                                     ...
                                     <strong>${otherSubjectVo.nonRFC822NAMEOptions[0]}</strong>
                                       ...
                                       <c:otherwise>
                            	        	<select class="span6 m-wrap" name="selectsubjectaltname${status.index}" data-placeholder="Choose a Category" tabindex="1">
												...
												<option value="${nonrfc822NameOption}"  ${otherSubjectVo.subjectDNField eq nonrfc822NameOption?"selected='selected'":""}>${nonrfc822NameOption}</option>
												  </c:forEach>
											</select>
                                            </c:otherwise>
											</c:choose>
     						     ...
     						     <c:otherwise>
   									 <c:if test="${otherSubjectVo.fieldOfTypeUPN}"> 
  									      	<input type="text" name="textfieldupnname${status.index}" size="20" maxlength="255" value="${name }"  class="span3 m-wrap popovers" data-trigger="hover" tabindex="" /> @
                               ...
                               value="${otherSubjectVo.subjectDNField}"  <c:if test="${not empty otherSubjectVo.regex}">pattern="${otherSubjectVo.regex}" </c:if>
 						     ...
 						     <c:forEach var="otherSubjectVoDirAttr" items="${dirAttrList}" varStatus="status">
                                     ...
                                     <strong>${otherSubjectVoDirAttr.nonRFC822NAMEOptions[0]}</strong>
                           	        ...
                           	        <c:otherwise> 
                            	        	<select class="span6 m-wrap" name="selectsubjectaldirattr${status.index}" data-placeholder="Choose a Category" tabindex="1">
												...
												<option value="${otherSubjectVoDirAttrOption}"   ${otherSubjectVoDirAttr.subjectDNField eq otherSubjectVoDirAttrOption?"selected='selected'":""} >${otherSubjectVoDirAttrOption}</option>
												  </c:forEach>
											</select>
                             	   </c:otherwise>
                                       </c:choose>
  								       ...
  								       <c:if test="${otherSubjectVoDirAttr.modifyable}">
  								         <input type="text" name="textfieldsubjectdirattr${status.index}"  class="span6 m-wrap popovers" data-trigger="hover" size="20" maxlength="255" tabindex="" value='${otherSubjectVoDirAttr.subjectDNField}'/>
   									  	</c:if>
    										${subjectDn.required?"*":""}
   										</div>
  									</div>
  						      ...
  						      <div class="control-group">
 									...
 									<div class="control-group">
 										...
 										<div class="controls">
   											<select class="span6 m-wrap" name="selectcertificateprofile" id="selectcertificateprofile" onchange="fillCAField()" data-placeholder="Choose a Category" tabindex="1">
												...
												<option value="${profileMap.key}"    ${endEntityProfileId eq profileMap.key?"selected='selected'":""} >${profileMap.value}</option>
												  </c:forEach>
											</select>
   											<b class="bclass"> *</b>
   										</div>
  									...
  									<div class="control-group">
 										...
 										<div class="controls">
    									    ...
    									    tabindex="1">
												...
												<option value="${tokens.value}"  ${tokens.value eq tokenType?"selected='selected'":""}   >${tokens.key}</option>
												  </c:forEach>
											</select>
    										<b class="bclass"> *</b>
   										</div>
 									  ...
 									  <hr />
 									   ...
 									   <c:if test="${usekeyrecovery}">
 										...
 										<div class="controls">
   									         <input type="checkbox" name="checkboxkeyrecoverable" id="checkboxkeyrecoverable"  value="true" tabindex=""   ${keyRecoverabl?"checked='checked'":""}   ${isRequiredUsekeyrecovery?"disabled='disabled'":""}  />
     										</div>
  									</div>
 									</c:if>
 		                      	...
 		                      	<div class="form-actions">
 										<button type="button" class="btn blue" onclick="changeUserData()">提交</button>
...
$(function () { 
	...
	fillCAFieldAndSelected($("#selectcertificateprofile").val(),'${caAndCertificateprofileRel}','${caId}');
 });
