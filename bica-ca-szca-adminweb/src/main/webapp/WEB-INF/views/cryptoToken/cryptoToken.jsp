				...
				<div class="span8">
					...
					<div class="portlet box blue">
						...
						<div class="portlet-body form">
							<!-- BEGIN FORM-->
							<form name="cryptForm" action="addCryptoToken.html" id="cryptForm" class="form-horizontal">
								<c:if test="${!empty cryptoTokenGuiInfo}">
									<div class="control-group">
										<label class="control-label">序号 </label>
										<div class="controls">
												${cryptoTokenGuiInfo.cryptoTokenId}
											<input type="hidden" name="cryptoTokenId" value="${cryptoTokenGuiInfo.cryptoTokenId}" />
										</div>
									</div>
								</c:if>
								<c:if test="${empty cryptoTokenGuiInfo}">
									<input type="hidden" name="cryptoTokenId" value="0" />
								</c:if>
								<div class="control-group">
									<label class="control-label">名称 </label>
									<div class="controls">
										<input type="text" name="name" required="required" value="${cryptoTokenGuiInfo.tokenName}" size="45" title="Identifier, string formated" />
									</div>
								</div>
								<div class="control-group">
								...
								<div class="control-group">
									...
									<div class="controls">
										<input id="authCode" type="password" <c:if test="${empty cryptoTokenGuiInfo}">required="required"</c:if> name="authCode" autocomplete="off" value="" size="20" title="Authentication Code" />
									</div>
								</div>
								<!--
                                <div class="control-group">
								...
								<div class="form-actions">
									...
									<c:if test="${!empty cryptoTokenGuiInfo}">
										<button type="reset" class="btn blue" onclick="deleteCryptoToken(${cryptoTokenGuiInfo.cryptoTokenId});">删除</button>
										<button type="button" class="btn blue" onclick="activeCryptoToken(${cryptoTokenGuiInfo.cryptoTokenId});">激活</button>
									</c:if>
							...
							<div class="portlet-body form">
								...
								<form name="cryptForm" action="genNewKeyPair.html" class="form-horizontal">
									...
									<div class="control-group">
										...
										<div class="controls">
											<input type="text" name="alias" value="" required="required" size="45" title="Identifier, string formated" />
										</div>
									</div>
									<div class="control-group">
...
<script>
    ...
    function deleteCryptoToken(cryptoTokenId){
        if(confirm("确定要删除密钥库?")){
            window.location.href ="${pageContext.request.contextPath}/cryptoToken/deleteCryptoToken.html?cryptoTokenId="+cryptoTokenId;
        }
    }
    function activeCryptoToken(cryptoTokenId){
        var authcode = $("#authCode").val();
        if(authcode == null || authcode.length < 1){
            alert('请输入授权码！');
        }else{
            $('#cryptForm').attr("action", "activeCryptoToken.html");
            $('#cryptForm').submit();
        }
    }
