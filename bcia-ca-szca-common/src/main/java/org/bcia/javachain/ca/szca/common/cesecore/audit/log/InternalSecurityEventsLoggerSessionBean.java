...
import org.bcia.javachain.ca.szca.common.cesecore.audit.impl.integrityprotected.IntegrityProtectedLoggerSessionLocal;
...
import org.bcia.javachain.ca.szca.common.cesecore.audit.impl.integrityprotected.IntegrityProtectedLoggerSessionBean;
//import org.bcia.javachain.ca.szca.common.cesecore.audit.impl.integrityprotected.AuditRecordData;
import org.bcia.javachain.ca.szca.common.cesecore.audit.impl.integrityprotected.IntegrityProtectedLoggerSessionLocal;
import org.bcia.javachain.ca.szca.common.cesecore.audit.impl.queued.QueuedLoggerSessionLocal;
 ...
 * 
 * @version $Id: InternalSecurityEventsLoggerSessionBean.java 20601 2015-01-27 07:24:40Z jeklund $
 */  
//@Stateless
        ...
        for (final String loggerId : AuditDevicesConfig.getAllDeviceIds()) {
            try {
               ...
               // AuditDevicesConfig.getDevice(ejbs, loggerId).log(trustedTime, eventType, eventStatus, module, service, authToken, customId, searchDetail1, searchDetail2, additionalDetails, AuditDevicesConfig.getProperties(loggerId));
                AuditLogDevice device = AuditDevicesConfig.getDevice(ejbs, loggerId);
                System.out.println("=========AuditLogDevice Class:"+device.getClass());
                //org.cesecore.audit.impl.integrityprotected.IntegrityProtectedDevice
                if("org.cesecore.audit.impl.integrityprotected.IntegrityProtectedDevice".equals(device.getClass().getName())){
                	integrityProtectedLoggerSession.log(trustedTime, eventType, eventStatus, module, service, authToken, customId, searchDetail1, searchDetail2, additionalDetails, AuditDevicesConfig.getProperties(loggerId));
                }else
                	
                	device.log(trustedTime, eventType, eventStatus, module, service, authToken, customId, searchDetail1, searchDetail2, additionalDetails, AuditDevicesConfig.getProperties(loggerId));
                if(LOG.isDebugEnabled()){
                    sw.stop();
        ...
        if (anyFailures) {
            ...
            if (!eventType.equals(EventTypes.LOG_VERIFY) && !eventType.equals(EventTypes.LOG_EXPORT) && !eventType.equals(EventTypes.LOG_DELETE) && !eventType.equals(EventTypes.LOG_SIGN)  && !eventType.equals(EventTypes.LOG_MANAGEMENT_CHANGE)) {
              //  throw new AuditRecordStorageException("Failed to write audit log to at least one device.");
            	LOG.error("==================Failed to write audit log to at least one device.");
            	System.out.println("==================Failed to write audit log to at least one device.");
            }
        }
    }
