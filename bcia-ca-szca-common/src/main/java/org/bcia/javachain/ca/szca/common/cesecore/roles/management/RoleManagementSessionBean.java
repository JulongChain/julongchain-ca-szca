 ...
 *                                                                       *
 *************************************************************************/
package org.bcia.javachain.ca.szca.common.cesecore.roles.management;
...
import org.bcia.javachain.ca.szca.common.cesecore.authorization.cache.AccessTreeUpdateSessionLocal;
import org.bcia.javachain.ca.szca.common.cesecore.authorization.control.AccessControlSessionLocal;
import org.bcia.javachain.ca.szca.common.cesecore.authorization.rules.AccessRuleManagementSessionLocal;
import org.bcia.javachain.ca.szca.common.cesecore.authorization.user.AccessUserAspectManagerSessionLocal;
...
public class RoleManagementSessionBean implements RoleManagementSessionLocal, RoleManagementSessionRemote {
    ...
    @Autowired
    private org.bcia.javachain.ca.szca.common.cesecore.roles.access.RoleAccessSessionLocal roleAccessSession;

    @Autowired
    private org.bcia.javachain.ca.szca.common.cesecore.audit.log.SecurityEventsLoggerSessionLocal securityEventsLogger;
    ...
    public RoleData replaceAccessRulesInRole(final AuthenticationToken authenticationToken, final RoleData role,
            final Collection<AccessRuleData> accessRules) throws AuthorizationDeniedException, RoleNotFoundException, AccessRuleExistsException {
        assertAuthorizedToEditRole(authenticationToken, role);
        //Check that current aspect is authorized to all the rules she's planning on replacing
        if(!isAuthorizedToRules(authenticationToken, accessRules)) {
            throw new AuthorizationDeniedException(authenticationToken + " not authorized to all access rules.");
        ...
        for (AccessRuleData rule : accessRules) {
            ...
            if (rulesFromResult.containsKey(ruleKey)) {
                ...
                try {
                    ...
                    throw new AccessRuleExistsException("Access rule exists, but wasn't found in persistence in previous call.", e);
                }
